...
async def generate_report_markdown(
    ...
    system_prompt = (
        ...
        "## Überblick\n"
        "Zusammenfassung: Welche Gesetze/Verordnungen wurden geändert, "
        "was sind die wesentlichen Neuerungen.\n\n"
        "## [Sprechender Titel der Änderung]\n"
        "Pro wesentliche Änderung ein eigener Abschnitt. Jeder Abschnitt "
        "muss KONKRET beschreiben:\n"
        "- WAS hat sich geändert (alter Rechtszustand vs. neuer Rechtszustand)\n"
        "- WELCHE Paragrafen/Bestimmungen betroffen sind\n"
        "- WARUM (wenn Materialien vorliegen: Intention des Gesetzgebers)\n"
        "- WER ist betroffen und WAS ist zu tun\n"
        "Jeder Abschnitt soll 5-8 Sätze haben — genug Detail für einen "
        "Juristen, der die Änderung verstehen will ohne selbst nachzulesen.\n\n"
        "## Ausblick\n"
        "Trends, geplante Änderungen, offene Punkte.\n\n"
        "REGELN:\n"
        "- Pro Abschnitt KONKRETE Inhalte, nicht nur 'es gab Änderungen'\n"
        "- Wenn VERSIONSVERGLEICHE vorliegen: beschreibe genau was sich im "
        "Wortlaut geändert hat (z.B. 'Der Freibetrag wurde von X auf Y erhöht')\n"
        "- Wenn MATERIALIEN vorliegen: fasse die Erläuterungen inhaltlich zusammen\n"
        "- BGBl-Nummer und Inkrafttreten im Fließtext nennen\n"
        "- Keine URL-Links, keine Quellenverzeichnisse\n"
        "- Fokus auf die rechtliche Substanz, nicht den Verfahrensgang"
    )

    user_prompt = (
        f"Analysiere die folgenden {total_hits} "
        f"Rechtsakte im Bereich '{category_label}' "
        f"({timeframe_label}). Pro wesentliche Änderung ein eigener "
        f"## Abschnitt. Beschreibe KONKRET was sich geändert hat "
        f"(alter vs. neuer Rechtszustand):\n\n{result_text}"
        f"{extra_context}"
    )
